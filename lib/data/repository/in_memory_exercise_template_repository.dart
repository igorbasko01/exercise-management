...
class InMemoryExerciseRepository extends ExerciseTemplateRepository {
  ...
  Future<Result<ExerciseTemplate>> addExercise(
    ...
    if (exercise.id == null) {
      final updatedTemplate = exercise.copyWith(id: Value(uniqueId()));
      exercise = updatedTemplate;
    }

    if (_exercises.contains(exercise)) {
      return Result.error(ExerciseAlreadyExistsException(
          'Exercise ${exercise.id} already exists'));
    }
  ...
  Future<Result<ExerciseTemplate>> deleteExercise(String id) {
    ...
    if (exerciseIndex == -1) {
      return Future.value(
          Result.error(ExerciseNotFoundException('Exercise $id not found')));
    }
  ...
  Future<Result<ExerciseTemplate>> updateExercise(
    ...
    _exercises[exerciseIndex] = exercise.copyWith(id: Value(storedExercise.id));
    return Result.ok(exercise);
  }
